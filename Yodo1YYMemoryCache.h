...
/**
 Yodo1YYMemoryCache is a fast in-memory cache that stores key-value pairs.
 ...
 Yodo1YYMemoryCache objects differ from NSCache in a few ways:
 ...
 * It can be configured to automatically evict objects when receive memory 
 ...
 The time of `Access Methods` in Yodo1YYMemoryCache is typically in constant time (O(1)).
 */
@interface Yodo1YYMemoryCache : NSObject
 ...
 */
@property (nullable, copy) void(^didReceiveMemoryWarningBlock)(Yodo1YYMemoryCache *cache);

/**
 A block to be executed when the app enter background.
 The default value is nil.
 */
@property (nullable, copy) void(^didEnterBackgroundBlock)(Yodo1YYMemoryCache *cache);
