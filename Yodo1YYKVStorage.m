...
static const NSUInteger kYodo1OPMaxErrorRetryCount = 8;
static const NSTimeInterval kYodo1OPMinRetryTimeInterval = 2.0;
static const int kYodo1OPPathLengthMax = PATH_MAX - 64;
static NSString *const kYodo1OPDBFileName = @"manifest.sqlite";
static NSString *const kYodo1OPDBShmFileName = @"manifest.sqlite-shm";
static NSString *const kYodo1OPDBWalFileName = @"manifest.sqlite-wal";
static NSString *const kYodo1OPDataDirectoryName = @"data";
static NSString *const kYodo1OPTrashDirectoryName = @"trash";
...
static UIApplication *_Yodo1YYSharedApplication() {
...
@implementation Yodo1YYKVStorageItem
@end

@implementation Yodo1YYKVStorage {
...
- (BOOL)_dbCheck {
    if (!_db) {
        if (_dbOpenErrorCount < kYodo1OPMaxErrorRetryCount &&
            CACurrentMediaTime() - _dbLastOpenErrorTime > kYodo1OPMinRetryTimeInterval) {
...
- (Yodo1YYKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    Yodo1YYKVStorageItem *item = [Yodo1YYKVStorageItem new];
...
- (Yodo1YYKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    Yodo1YYKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            Yodo1YYKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
 ...
 */
- (void)_reset {
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:kYodo1OPDBFileName] error:nil];
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:kYodo1OPDBShmFileName] error:nil];
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:kYodo1OPDBWalFileName] error:nil];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"Yodo1YYKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:@"" type:Yodo1YYKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(Yodo1YYKVStorageType)type {
    if (path.length == 0 || path.length > kYodo1OPPathLengthMax) {
        NSLog(@"Yodo1YYKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > Yodo1YYKVStorageTypeMixed) {
        NSLog(@"Yodo1YYKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
    ...
    _path = path.copy;
    _type = type;
    _dataPath = [path stringByAppendingPathComponent:kYodo1OPDataDirectoryName];
    _trashPath = [path stringByAppendingPathComponent:kYodo1OPTrashDirectoryName];
    _trashQueue = dispatch_queue_create("com.ibireme.cache.disk.trash", DISPATCH_QUEUE_SERIAL);
    _dbPath = [path stringByAppendingPathComponent:kYodo1OPDBFileName];
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error] ||
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kYodo1OPDataDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error] ||
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kYodo1OPTrashDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
        NSLog(@"Yodo1YYKVStorage init error:%@", error);
        return nil;
    }
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
            NSLog(@"Yodo1YYKVStorage init error: fail to open sqlite db.");
            return nil;
        }
    }
...
- (void)dealloc {
    UIBackgroundTaskIdentifier taskID = [_Yodo1YYSharedApplication() beginBackgroundTaskWithExpirationHandler:^{}];
    [self _dbClose];
    if (taskID != UIBackgroundTaskInvalid) {
        [_Yodo1YYSharedApplication() endBackgroundTask:taskID];
    }
}

- (BOOL)saveItem:(Yodo1YYKVStorageItem *)item {
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == Yodo1YYKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != Yodo1YYKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case Yodo1YYKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case Yodo1YYKVStorageTypeFile:
        case Yodo1YYKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (Yodo1YYKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (Yodo1YYKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    Yodo1YYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (Yodo1YYKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    Yodo1YYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
...
- (NSArray *)getItemForKeys:(NSArray *)keys {
    ...
    if (_type != Yodo1YYKVStorageTypeSQLite) {
        for (NSInteger i = 0, max = items.count; i < max; i++) {
            Yodo1YYKVStorageItem *item = items[i];
            if (item.filename) {
                item.value = [self _fileReadWithName:item.filename];
                if (!item.value) {
